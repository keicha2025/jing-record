...
export const SearchBar = {
    ...
    <div class="sticky top-0 z-40 bg-[#FDFCFB]/95 backdrop-blur-sm py-2 -mx-4 px-4 border-b border-gray-50/50">
        <div class="flex space-x-2 h-11">
            <!-- Search Input -->
            <div class="flex-1 bg-white rounded-xl flex items-center px-3 shadow-sm border border-gray-50 transition-shadow focus-within:shadow-md h-full">
            ...
            <div class="w-28 h-full">
                <app-select 
                    ...
                    rounded-class="rounded-xl h-full"
                ></app-select>
            </div>
        </div>
    </div>
    ...
    methods: {
        updateMode(newMode) {
            this.$emit('update:modelValue', { ...this.modelValue, mode: newMode });
        }
    }
};
